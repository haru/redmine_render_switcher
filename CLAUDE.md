...
**Current state: released as v0.1.0; the detector rules are being revised on `bugfix/002-fix-detector-rules` (ADR-0005).** The full suite passes (164 runs), RuboCop is clean, YARD is at 100% and C0 coverage of `lib/` is 100%. New work is changes to a working plugin, not a greenfield build.

## Architecture

The whole plugin is ~900 lines (mostly YARD comments) in `lib/redmine_render_switcher/`, wired up from `init.rb`. It couples to Redmine core at exactly **two points** (ADR-0001, ADR-0002, ADR-0004):
...
- **Bump `Detector::VERSION`** in the same change as anything that can move a verdict: the pattern tables, any of the four line-context rules (`#` lines, lists, tables, Setext headings), code-block masking (fences, `<pre>` and indented blocks), the threshold comparison, or `Directive::PATTERN`. Forgetting is a defect — stale cached HTML keeps being served. The threshold is a setting, so `CacheKey` handles it separately.
...
- **Detector ambiguity is resolved with context rules, not more patterns.** Textile's `# item` numbered list collides with Markdown's `h1`; two or more `#` lines in a row score Textile, a lone one scores Markdown. Notation both formats read identically is deliberately unscored. The list and table rules score once per block, never per line, so evidence does not grow with length (a Textile `h1.` document with five dash lines stays 5:3), and a list needs two lines or more. The Setext rule is the one exception: it scores each heading, so a Textile document with two `=` underlines reads as Markdown (accepted, ADR-0005). A `#` line is never a list item for the purpose of masking an indented code block, and only a row made purely of pipes, hyphens, colons and spaces is a table separator. The score margin is clamped to ≥ 1 so a tie never picks a side. `Result` never carries the text, so debug logs cannot leak page content.
